...
$stmt = $db->prepare("SELECT * FROM appointments WHERE doctor_name = ? AND prescription IS NOT NULL AND prescription != '' ORDER BY appointment_date DESC");
...
<html lang="en">
<head>
    <meta charset="UTF-8"><title>Prescriptions | Doctor Portal</title>
    <style>
...
<body>
    ...
    <main class="main-content">
        <h2 style="margin-bottom: 20px;">💊 Written Prescriptions</h2>
        <div class="panel">
            <table>
                <tr style="background: #f8fafc;"><th>Date Issued</th><th>Patient Name</th><th>Medicines Prescribed</th></tr>
                <?php foreach ($prescriptions as $p): ?>
                <tr>
                    <td style="color:#64748b;"><?= date("M j, Y", strtotime($p['appointment_date'])) ?></td>
                    <td><strong><?= htmlspecialchars($p['patient_name']) ?></strong></td>
                    <td style="color:#0f172a; background: #f8fafc; padding: 10px; border-radius: 6px; font-family: monospace;"><?= nl2br(htmlspecialchars($p['prescription'])) ?></td>
                </tr>
                <?php endforeach; ?>
            </table>
        </div>
